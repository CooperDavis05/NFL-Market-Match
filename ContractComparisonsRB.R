...
RB_Stats <- rushing_summary_2017_2024 |> left_join(receiving_summary_2017_2024 |> select(player_id, season, position, team_name, player, touchdowns, pass_plays), 
                                                     by = c("player_id", "season", "position", "team_name", "player")) |> 
                                         left_join(offense_blocking_2018_2024 |> select(player_id, season, position, team_name, player, snap_counts_offense), 
                                                   by = c("player_id", "season", "position", "team_name", "player")) |> 
  summarize(player, player_id, position, season, team_name, 
            grades_offense,
            attempts, rushing_yards = designed_yards, rec_yards, targets, touchdowns = touchdowns.x + touchdowns.y,
            snaps = snap_counts_offense) |> 
  filter(position %in% c("HB")) |> unique()



# Use nflverse rosters to filter position
rosters2018_2024_RB <- rosters2018_2024 |> ungroup() |> filter(depth_chart_position %in% c("RB", "HB"))



rosters_RB_Contract <- rosters2018_2024_RB |> 
...
RB_contracts <- rosters_RB_Contract |> 
  left_join(RB_Stats |> mutate(player = clean_player_names(player)), 
...
rosters_RB_Contract_2024 <- rosters2018_2024_RB |> 
...
free_agents_RB_2024 <- read.csv("FreeAgents2025.csv") |> 
  filter(sortable.3 %in% c("SFA", "UFA", "Void")) |> 
  filter(sortable.2 %in% c("RB")) |> 
  summarize(full_name = clean_player_names(sortable)) |>
  left_join(rosters_RB_Contract_2024, by = "full_name") |> 
...
RB_contracts_2024 <- free_agents_RB_2024 |> 
  left_join(RB_Stats |> filter(season == 2024) |> filter(position %in% c("HB")) |> mutate(player = clean_player_names(player)), 
...
RB_contracts_combined <- bind_rows(RB_contracts, RB_contracts_2024) |> 
  summarize(full_name, gsis_id, team, position, 
            ...
            grades_offense,
            attempts, rushing_yards, rec_yards, targets, touchdowns,
            rolling_3_snaps,
            FA_2025) |> 
...
RB_contracts_combined_forknn <- RB_contracts_combined |> 
...
RB_contracts_combined_forknn_normalized <- RB_contracts_combined_forknn %>%
  mutate(
...
train_data <- RB_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(RB_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 0) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))


test_data <- RB_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(RB_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 1) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))
...
everything_matched_RB <- top_matches_named |> left_join(RB_contracts, 
...
everything_matched_RB_2024 <- RB_contracts_2024 |> 
...
write.csv(everything_matched_RB, "RBMatched.csv")
write.csv(everything_matched_RB_2024, "RBID.csv")
