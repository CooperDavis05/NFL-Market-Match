...
LB_Stats <- defense_coverage_summary_2017_2024 |> left_join(run_defense_summary_2017_2024 |> select(player_id, season, position, team_name, player, snap_counts_run, forced_fumbles), 
                                                     by = c("player_id", "season", "position", "team_name", "player")) |> 
  summarize(player, player_id, position, season, team_name, grades_defense, grades_run_defense, forced_incompletes, 
            grades_coverage_defense, 
            grades_tackle, missed_tackles, pass_break_ups,
            snaps = snap_counts_run + snap_counts_pass_play, 
            forced_fumbles, stops, tackles) |> 
  filter(position %in% c("LB"))

# Use nflverse rosters to filter position
rosters2018_2024_LB <- rosters2018_2024 |> ungroup() |> filter(depth_chart_position %in% c("OLB", "MLB", "ILB", "LB"))



rosters_LB_Contract <- rosters2018_2024_LB |> 
...
LB_contracts <- rosters_LB_Contract |> 
  left_join(LB_Stats |> mutate(player = clean_player_names(player)), 
...
rosters_LB_Contract_2024 <- rosters2018_2024_LB |> 
...
LB_contracts_2024 <- free_agents_LB_2024 |> 
  left_join(LB_Stats |> filter(season == 2024) |> filter(position %in% c("LB")) |> mutate(player = clean_player_names(player)), 
...
LB_contracts_combined <- bind_rows(LB_contracts, LB_contracts_2024) |> 
  summarize(full_name, gsis_id, team, position, 
            ...
            grades_defense, grades_run_defense, forced_incompletes, 
            grades_coverage_defense, 
            grades_tackle, missed_tackles, pass_break_ups, rolling_3_snaps, 
            forced_fumbles, stops, tackles, FA_2025) |> 
  group_by(full_name, year_signed) %>%  
  slice_max(order_by = value, n = 1, with_ties = FALSE) %>%  
  ungroup()

# Select the columns for use and turn non-numeric variables into numeric or dummy
LB_contracts_combined_forknn <- LB_contracts_combined |> 
  summarize(full_name, team, position, year_signed, height, weight, draft_overall, contract_number, age_at_signing, snaps, 
            #grades_defense, 
            #grades_run_defense, forced_incompletes, 
            grades_coverage_defense, 
            grades_tackle, tackles, missed_tackles, pass_break_ups, rolling_3_snaps, 
            #forced_fumbles, 
            stops, FA_2025) %>%
...
LB_contracts_combined_forknn_normalized <- LB_contracts_combined_forknn %>%
  mutate(
...
train_data <- LB_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(LB_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 0) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))

test_data <- LB_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(LB_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 1) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))
...
everything_matched_LB <- top_matches_named |> left_join(LB_contracts, 
  ...
  summarize(Player_Source, Match_Num, Match2, Match_year, Score, team, position, height, weight, draft_overall, contract_number, age_at_signing, 
            ...
            #grades_run_defense, forced_incompletes, 
            grades_coverage_defense, 
            grades_tackle, tackles, missed_tackles, pass_break_ups,
            #forced_fumbles, 
            rolling_3_snaps,
            stops, FA_2025) |>
...
everything_matched_LB_2024 <- LB_contracts_2024 |> 
...
write.csv(everything_matched_LB, "LBMatched.csv")
write.csv(everything_matched_LB_2024, "LBID.csv")
