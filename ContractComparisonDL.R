...
pass_rush_summary_2024 <- read.csv("pass_rush_summary.csv") |> mutate(season = 2024)
pass_rush_summary_2023 <- read.csv("pass_rush_summary (1).csv") |> mutate(season = 2023)
pass_rush_summary_2022 <- read.csv("pass_rush_summary (2).csv") |> mutate(season = 2022)
pass_rush_summary_2021 <- read.csv("pass_rush_summary (3).csv") |> mutate(season = 2021)
pass_rush_summary_2020 <- read.csv("pass_rush_summary (4).csv") |> mutate(season = 2020)
pass_rush_summary_2019 <- read.csv("pass_rush_summary (5).csv") |> mutate(season = 2019)
pass_rush_summary_2018 <- read.csv("pass_rush_summary (6).csv") |> mutate(season = 2018)
pass_rush_summary_2017 <- read.csv("pass_rush_summary (7).csv") |> mutate(season = 2017)

pass_rush_summary_2017_2024 <- rbind(pass_rush_summary_2024, pass_rush_summary_2023, pass_rush_summary_2022,
                                    pass_rush_summary_2021, pass_rush_summary_2020, pass_rush_summary_2019,
                                    pass_rush_summary_2019, pass_rush_summary_2018, pass_rush_summary_2017)
...
DL_Stats <- pass_rush_summary_2017_2024 |> left_join(run_defense_summary_2017_2024, 
                                                     by = c("player_id", "season", "position", "team_name", "player")) |> 
  summarize(player, player_id, position, season, team_name, grades_defense, grades_pass_rush_defense = grades_pass_rush_defense.x, grades_run_defense,
            pass_rush_win_rate, sacks, total_pressures, snaps = snap_counts_run + snap_counts_pass_play, pressure_pct = total_pressures/snap_counts_pass_rush,
            forced_fumbles, stops) |> 
  filter(position %in% c("DI", "ED"))

# Use nflverse rosters to filter position
rosters2018_2024_DL <- rosters2018_2024 |> ungroup() |> filter(depth_chart_position %in% c("OLB", "DE", "DL", "DT", "NT"))
  


rosters_DL_Contract <- rosters2018_2024_DL |> 
...
DL_contracts <- rosters_DL_Contract |> 
  left_join(DL_Stats |> mutate(player = clean_player_names(player)), 
            by = c("full_name" = "player", "ybs" = "season")) |> 
  filter(!is.na(pass_rush_win_rate)) |> 
  unique() |> 
...
rosters_DL_Contract_2024 <- rosters2018_2024_DL |> 
  ...
  left_join(player_snap_counts |> select(player, season, rolling_3_snaps) |> mutate(season = ifelse((player == "Malcolm Koonce" & season == 2023), 2024, season)) |> filter(season == 2024) |> select(-season), by = c("full_name" = "player")) 

# Load in the free agent data and combine with measurables and such (no contracts included because they haven't signed yet)
free_agents_DL_2024 <- read.csv("FreeAgents2025.csv") |> 
  filter(sortable.3 %in% c("SFA", "UFA", "Void", "Franchise")) |> 
  filter(sortable.2 %in% c("EDGE", "IDL")) |> 
  summarize(full_name = clean_player_names(sortable)) |>
  left_join(rosters_DL_Contract_2024, by = "full_name") |> 
...
DL_contracts_2024 <- free_agents_DL_2024 |> 
  left_join(DL_Stats |> filter(season == 2024 | (player == "Malcolm Koonce" & season == 2023)) |> filter(position %in% c("ED", "DI")) |> mutate(player = clean_player_names(player)), 
...
DL_contracts_combined <- bind_rows(DL_contracts, DL_contracts_2024) |> 
  summarize(full_name, gsis_id, team, position, 
            ...
            cap = case_when(
              is.na(cap) ~ 279.2,
              TRUE ~ cap
            ), team_name, snaps, grades_defense, grades_pass_rush_defense, grades_run_defense, 
            PRWR = pass_rush_win_rate, sacks, total_pressures, pressure_pct = round(pressure_pct, 2), forced_fumbles, stops, rolling_3_snaps, FA_2025) |> 
  group_by(full_name, year_signed) %>%  
  slice_max(order_by = value, n = 1, with_ties = FALSE) %>%  
  ungroup()

# Select the columns for use and turn non-numeric variables into numeric or dummy
DL_contracts_combined_forknn <- DL_contracts_combined |> 
  summarize(full_name, team, position, year_signed, height, weight, draft_overall, contract_number, age_at_signing, 
            snaps, grades_defense, #grades_pass_rush_defense, grades_run_defense, 
            PRWR, sacks, total_pressures, 
            rolling_3_snaps,
            #pressure_pct, forced_fumbles, 
            stops, FA_2025) %>%
...
DL_contracts_combined_forknn_normalized <- DL_contracts_combined_forknn %>%
  mutate(
...
train_data <- DL_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(DL_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 0) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))

test_data <- DL_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(DL_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 1) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))
...
everything_matched_DL <- top_matches_named |> left_join(DL_contracts, 
  ...
  summarize(Player_Source, Match_Num, Match2, Match_year, Score, team, position, height, weight, draft_overall, contract_number, age_at_signing, 
            years, value, apy, apy_cap_pct, inflated_apy = apy_cap_pct*279.2, guaranteed, snaps, grades_defense, #grades_pass_rush_defense, grades_run_defense, 
            PRWR = pass_rush_win_rate, sacks, total_pressures, rolling_3_snaps, 
            #pressure_pct, forced_fumbles, 
            stops, FA_2025) |>
...
write.csv(everything_matched_DL, "DLMatched.csv")
write.csv(everything_matched_DL_2024, "DLID.csv")
