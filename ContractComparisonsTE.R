...
TE_Stats <- receiving_summary_2017_2024 |> left_join(offense_blocking_2018_2024, 
                                                           by = c("player_id", "season", "position", "team_name", "player")) |> 
  summarize(player, player_id, position, season, team_name, 
            grades_offense = grades_offense.x, grades_pass_block = grades_pass_block.x, grades_run_block,
            receptions, targets, yards, touchdowns, yprr,
...
rosters2018_2024_TE <- rosters2018_2024 |> ungroup() |> filter(position %in% c("TE"))



rosters_TE_Contract <- rosters2018_2024_TE |> 
...
TE_contracts <- rosters_TE_Contract |> 
  left_join(TE_Stats |> mutate(player = clean_player_names(player)), 
...
rosters_TE_Contract_2024 <- rosters2018_2024_TE |> 
...
free_agents_TE_2024 <- read.csv("FreeAgents2025.csv") |> 
  filter(sortable.3 %in% c("SFA", "UFA", "Void", "Franchise")) |> 
  filter(sortable.2 %in% c("TE")) |> 
  summarize(full_name = clean_player_names(sortable)) |>
  left_join(rosters_TE_Contract_2024, by = "full_name") |> 
...
TE_contracts_2024 <- free_agents_TE_2024 |> 
  left_join(TE_Stats |> filter(season == 2024) |> filter(position %in% c("TE")) |> mutate(player = clean_player_names(player)), 
...
TE_contracts_combined <- bind_rows(TE_contracts, TE_contracts_2024) |> 
...
TE_contracts_combined_forknn <- TE_contracts_combined |> 
...
TE_contracts_combined_forknn_normalized <- TE_contracts_combined_forknn %>%
  mutate(
...
train_data <- TE_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(TE_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 0) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))


test_data <- TE_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(TE_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 1) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))
...
everything_matched_TE <- top_matches_named |> left_join(TE_contracts, 
...
everything_matched_TE_2024 <- TE_contracts_2024 |> 
...
write.csv(everything_matched_TE, "TEMatched.csv")
write.csv(everything_matched_TE_2024, "TEID.csv")
