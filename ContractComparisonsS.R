...
S_Stats <- defense_coverage_summary_2017_2024 |> left_join(run_defense_summary_2017_2024 |> select(player_id, season, position, team_name, player, snap_counts_run, forced_fumbles), 
  ...
  summarize(player, player_id, position, season, team_name, 
            ...
            interceptions, missed_tackles, pass_break_ups,
            snaps = snap_counts_run + snap_counts_pass_play, 
            forced_fumbles, tackles) |> 
  filter(position %in% c("S"))

  

# Use nflverse rosters to filter position
rosters2018_2024_S <- rosters2018_2024 |> ungroup() |> filter(depth_chart_position %in% c("S", "SAF", "SS", "FS", "DB"))



rosters_S_Contract <- rosters2018_2024_S |> 
...
S_contracts <- rosters_S_Contract |> 
  left_join(S_Stats |> mutate(player = clean_player_names(player)), 
...
rosters_S_Contract_2024 <- rosters2018_2024_S |> 
...
free_agents_S_2024 <- read.csv("FreeAgents2025.csv") |> 
  filter(sortable.3 %in% c("SFA", "UFA", "Void", "Franchise")) |> 
  filter(sortable.2 %in% c("S")) |> 
  summarize(full_name = clean_player_names(sortable)) |>
  left_join(rosters_S_Contract_2024, by = "full_name") |> 
  mutate(contract_number = case_when(
    contract_number == "1" ~ "2",
    contract_number == "2" ~ "3+",
    TRUE ~ contract_number
  )) |> filter(position == "S") |> # Added
  select(-season) # Added


# Merge with the PFF Data and label as the training data
S_contracts_2024 <- free_agents_S_2024 |> 
  left_join(S_Stats |> filter(season == 2024) |> filter(position %in% c("S")) |> mutate(player = clean_player_names(player)), 
...
S_contracts_combined <- bind_rows(S_contracts, S_contracts_2024) |> 
  summarize(full_name, gsis_id, team, position, 
            ...
            grades_coverage_defense, 
            interceptions, missed_tackles, pass_break_ups,
            forced_fumbles, tackles, rolling_3_snaps, #Added
            FA_2025) |> 
...
S_contracts_combined_forknn <- S_contracts_combined |> 
...
S_contracts_combined_forknn_normalized <- S_contracts_combined_forknn %>%
  mutate(
...
weights <- c(
  ...
  age_at_signing = 7,
  snaps = 3,
  grades_run_defense = 6,
  grades_coverage_defense = 6,
  tackles = 3,
  missed_tackles = 1,
  pass_break_ups = 1,
  interceptions = 2,
  rolling_3_snaps = 3, # Added
  position_S = 1
)
...
train_data <- S_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(S_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 0) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))


test_data <- S_contracts_combined_forknn_normalized[,c(1,3, 4:ncol(S_contracts_combined_forknn_normalized))] %>% filter(FA_2025 == 1) %>% dplyr::select(-FA_2025)%>%
  mutate(across(names(weights), ~ . * weights[cur_column()], .names = "weighted_{col}"))
...
everything_matched_S <- top_matches_named |> left_join(S_contracts, 
...
everything_matched_S_2024 <- S_contracts_2024 |> 
...
write.csv(everything_matched_S, "SMatched.csv")
write.csv(everything_matched_S_2024, "SID.csv")
